...
# sstie_file = '/home/xinsheng/enigma/output/auto/mcmc/save_logM_9.90_R_1.00_logZ_-3.60_k_3.fits'
...
#outpath_plot = '/home/xinsheng/enigma/output/mcmc_compare/' # only for compare plots
linear_prior = False
cov = True
...
### calculate the probability for the most possible logZ, I commented it out so don't worry about it
    ...
    plt.title('Probability distribution for logZ = %.2f' % logZ_fine[logZ_max], fontsize=30, y=1.02)
    ...
    plt.yticks(logM_range,fontsize=20)
    plt.xticks(R_range, fontsize=20)
    plt.ylabel('logM', fontsize=25)
    plt.xlabel('R_Mpc', fontsize=25)

    cbar = plt.colorbar(im,fraction=0.046, pad=0.04)
    cbar.set_label('lnL', size=25)
...
if ( os.path.isfile(outpath_local + 'save_logM_%.2f_R_%.2f_logZ_%.2f_k_%d.fits' % (logM_guess, R_guess, logZ_guess, k)) == False ):
    ...
    sampler, param_samples, bounds = ie.mcmc_inference(walklength, 1000, nwalker, logM_fine_cov, R_fine_cov, logZ_fine_cov, lnlike_fine_cov, linear_prior, savefits_chain = chain_output, backend = backend, nproc = nproc)
...
# ie.plot_mcmc(sampler, param_samples, init_out, params, logM_fine_cov, R_fine_cov, logZ_fine_cov, xi_model_fine_cov, linear_prior, outpath_plot, overplot=overplot, overplot_param=param_sstie, fvfm_file=fvfm_file)
