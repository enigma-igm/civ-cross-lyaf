...
def compute_model_metal(args):
    ...
    vel_lores, (flux_lores, flux_lores_igm, flux_lores_cgm), vel_hires, (flux_hires, flux_hires_igm, flux_hires_cgm), \
    (oden, v_los, T, x_metal), cgm_tup = utils.create_metal_forest(params, skewers, logZ, fwhm, metal_ion=metal_ion, sampling=sampling, cgm_dict=None)

    vel_lores_lya, (flux_lores_lya, flux_lores_igm_lya, flux_lores_cgm_lya), vel_hires_lya, (flux_hires_lya, flux_hires_igm_lya, flux_hires_cgm_lya), \
    (oden_lya, v_los_lya, T_lya), cgm_tup_lya, tau_plot_lya = CIV_lya.create_lya_forest(params_lya, skewers_lya, fwhm, sampling=sampling, cgm_dict=None)

    # Add noise to the total flux
    print("Adding random noise to forest skewers ...")
    noise = rand.normal(0.0, 1.0 / SNR, flux_hires.shape)
    flux_noise = flux_hires + noise

    noise_lya = rand.normal(0.0, 1.0 / SNR, flux_hires_lya.shape)
    flux_noise_lya = flux_hires_lya + noise_lya

    # Compute delta_f
    print("Generating delta fields ...")
    mean_flux = np.mean(flux_noise) # flux + random noise
    delta_f = (flux_noise - mean_flux) / mean_flux
    mean_flux_nless = np.mean(flux_hires) # flux only
    delta_f_nless = (flux_hires - mean_flux_nless) / mean_flux_nless

    mean_flux_lya = np.mean(flux_noise_lya) # flux + random noise
    delta_f_lya = (flux_noise_lya - mean_flux_lya) / mean_flux_lya
    mean_flux_nless_lya = np.mean(flux_hires_lya) # flux only
    delta_f_nless_lya = (flux_hires_lya - mean_flux_nless_lya) / mean_flux_nless_lya
